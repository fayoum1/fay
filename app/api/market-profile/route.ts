...
export async function GET(request: NextRequest) {
  ...
  const { data: user, error } = await client.from("market_users").select("id,display_name,role,account_type,profile_image_url,cover_image_url,referral_code").eq("referral_code", code).eq("account_type", "market").eq("active", true).maybeSingle();
