...
صفحة «البيع» متاحة على `/sell`، وتستقبل عروض المزارع والشركات مع صورة اختيارية. شغّل النسخة المحدثة من `supabase/schema.sql` لإنشاء جدول `seller_offers` وإحصاءات زوار صفحة البيع وbucket باسم `seller-images`. تظهر العروض داخل تبويب «البائعون» في لوحة الأدمن.
