...
export async function authenticateMarketUser(phone: string, password: string) {
  ...
  const { data } = await client.from("market_users").select("id,display_name,phone,password_hash,role,account_type,receive_offers,referral_code,profile_image_url,cover_image_url,wallet_number").eq("phone", phone).eq("active", true).maybeSingle();
...
export async function getMarketUser(request: NextRequest) {
  ...
  const client = database();
  if (!client) return null;
  const { data } = await client.from("market_users").select("id,display_name,phone,password_hash,role,account_type,receive_offers,referral_code,profile_image_url,cover_image_url,wallet_number").eq("id", id).eq("active", true).maybeSingle();
